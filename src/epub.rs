...
pub fn gen_epub(novel: MangaSource, manga_metadata: MangaMetadata) -> Result<(), Box<dyn Error>> {
    let file_name = &gen_filename();
    let lang = &manga_metadata.manga_language;
    let epub_css = gen_css_from_lang(lang);
    let mut epub_builder = EpubBuilder::new(ZipLibrary::new().unwrap()).unwrap();
    epub_builder.metadata("title", &manga_metadata.manga_name)?;
    epub_builder.metadata("author", &manga_metadata.manga_author)?;
            ...
            .add_content(
                EpubContent::new(
                    ...
                    compose_html(&current_chapter.content, 
                        ...
                        &manga_metadata.manga_language).as_bytes(),
                )
